...
import { CreateOptionInput, AddChoiceInput, UpdateOptionInput, UpdateChoiceInput } from "./option.types";
...
export const optionController = (app: any) => {
  ...
  return {
    ...
    updateOption: async (
      req: FastifyRequest<{ Params: { id: string }; Body: UpdateOptionInput }>,
      reply: FastifyReply
    ) => {
      ...
      const isOwner = await verifyOwnership(app, req.user!.id, restaurantId);
      if (!isOwner) return reply.code(403).send({ error: 'Not your restaurant' });
      const updated = await service.updateOption(req.params.id, req.body);
      return reply.send(updated);
    },

    updateChoice: async (
      req: FastifyRequest<{ Params: { id: string }; Body: UpdateChoiceInput }>,
      reply: FastifyReply
    ) => {
      ...
      const isOwner = await verifyOwnership(app, req.user!.id, restaurantId);
      if (!isOwner) return reply.code(403).send({ error: 'Not your restaurant' });
      const updated = await service.updateChoice(req.params.id, req.body);
      return reply.send(updated);
    },
